...
func main() {
	redisConfigFn := redisconfig.NewStandaloneConfig()
	...
	standalone := redis.NewDefaultStandalone(logger, redisConfig)
	ctx := context.Background()

	_, _ = standalone.Set(ctx, "hello", "world", time.Hour)

	result, err := standalone.Get(ctx, "hello")
	if err != nil {
		logger.Fatal().Err(err).Send()
	}
