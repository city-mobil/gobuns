...
func TestWith(t *testing.T) {
	...
	log.Log().Msg("")
	if got, want := out.String(), `{"string":"foo","stringer":"127.0.0.1","stringer_nil":null,"bytes":"bar","hex":"12ef","json":{"some":"json"},"error":"some error","bool":true,"int":1,"int8":2,"int16":3,"int32":4,"int64":5,"uint":6,"uint8":7,"uint16":8,"uint32":9,"uint64":10,"float32":11.101,"float64":12.30303,"time":"0001-01-01T00:00:00Z","caller":"`+caller+`"}`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
	...
	log = ctx.CallerWithSkipFrameCount(4).Logger()
	func() {
		log.Log().Msg("")
	}()
...
func TestFieldsMap(t *testing.T) {
	...
	if got, want := out.String(), `{"bool":true,"bytes":"bar","dur":1000,"error":"some error","float32":11,"float64":12,"int":1,"int16":3,"int32":4,"int64":5,"int8":2,"ipv6":"2001:db8:85a3::8a2e:370:7334","nil":null,"string":"foo","time":"0001-01-01T00:00:00Z","uint":6,"uint16":8,"uint32":9,"uint64":10,"uint8":7}`+"\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestWithAndFieldsCombined(t *testing.T) {
	out := &bytes.Buffer{}
	log := Raw(out).With().Str("f1", "val").Str("f2", "val").Logger()
...
func TestGetLevel(t *testing.T) {
	...
	for _, level := range levels {
		if got, want := Raw(nil).Level(level).GetLevel(), level; got != want {
...
func TestSampling(t *testing.T) {
	out := &bytes.Buffer{}
	log := Raw(out).Sample(NewBasicSampler(2))
	log.Log().Int("i", 1).Msg("")
	log.Log().Int("i", 2).Msg("")
	log.Log().Int("i", 3).Msg("")
	log.Log().Int("i", 4).Msg("")
	if got, want := out.String(), "{\"i\":1}\n{\"i\":3}\n"; got != want {
		t.Errorf("invalid log output:\ngot:  %v\nwant: %v", got, want)
	}
}
...
func TestContextTimestamp(t *testing.T) {
	SetTimestampFunc(func() time.Time {
		return time.Date(2001, time.February, 3, 4, 5, 6, 7, time.UTC)
	})
	defer SetTimestampFunc(time.Now)

	out := &bytes.Buffer{}
	log := Raw(out).With().Timestamp().Str("foo", "bar").Logger()
...
func TestContextTimestamp_NewLogger(t *testing.T) {
	SetTimestampFunc(func() time.Time {
		return time.Date(2001, time.February, 3, 4, 5, 6, 7, time.UTC)
	})
	defer SetTimestampFunc(time.Now)
...
func TestEventTimestamp(t *testing.T) {
	...
	out := &bytes.Buffer{}
	log := Raw(out).With().Str("foo", "bar").Logger()
...
func TestOutputWithoutTimestamp(t *testing.T) {
	...
	out := &bytes.Buffer{}
	log := Raw(ignoredOut).Output(out).With().Str("foo", "bar").Logger()
	log.Log().Msg("hello world")
...
func TestOutputWithTimestamp(t *testing.T) {
	...
	ignoredOut := &bytes.Buffer{}
	out := &bytes.Buffer{}
	log := Raw(ignoredOut).Output(out).With().Timestamp().Str("foo", "bar").Logger()
	log.Log().Msg("hello world")
...
func (l loggableError) MarshalZerologObject(e *zerolog.Event) {
...
func TestErrorMarshalFunc(t *testing.T) {
	...
	defer SetErrorMarshalFunc(originalErrorMarshalFunc)

	SetErrorMarshalFunc(func(err error) interface{} {
...
func TestCallerMarshalFunc(t *testing.T) {
	...
	// test default behavior this is really brittle due to the line numbers
	...
	defer SetCallerMarshalFunc(origCallerMarshalFunc)
	SetCallerMarshalFunc(func(file string, line int) string {
...
func TestLevelFieldMarshalFunc(t *testing.T) {
	...
	SetLevelFieldMarshalFunc(func(l Level) string {
		return strings.ToUpper(l.String())
	})
	defer SetLevelFieldMarshalFunc(origLevelFieldMarshalFunc)
